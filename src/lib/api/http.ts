...
async function viaBridge(method: string, path: string, body: unknown, token: string | null) {
  return window.nerz!.api.request(method, path, body, token);
}

async function viaFetch(method: string, path: string, body: unknown, token: string | null) {
...
export async function apiRequest<T = unknown>(
  ...
  const { ok, status, data } = hasBridge
    ? await viaBridge(method, path, body ?? null, token ?? null)
    : await viaFetch(method, path, body ?? null, token ?? null);
